...
operater.getFriendList = function(uid,cb){
    var sql = "select tf.*,u.name,d.head_img,d.head_url,u.sex,u.birthday,u.user_id uid from talk_friend_table tf left join user_table u on u.user_id=tf.fid left join user_detail_table d on tf.fid=d.user_id where tf.uid=? group by fid order by tf.sort desc";
